...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    execute('UPDATE settings SET company_name=?, company_address=?, company_phone=?, company_email=?, default_tax_percent=?, currency=?, contract_footer=?, pdf_header=?, ceremony_types=?, updated_at=NOW() WHERE id=1',
        [$_POST['company_name'], $_POST['company_address'] ?? null, $_POST['company_phone'] ?? null, $_POST['company_email'] ?? null,
         $_POST['default_tax_percent'], $_POST['currency'], $_POST['contract_footer'] ?? null, $_POST['pdf_header'] ?? null, $_POST['ceremony_types'] ?? null]);
...
<form method="post">
    <div class="grid-2">
        <div class="card">
            ...
            <div class="form-group"><label>Address</label><textarea name="company_address" placeholder="Full business address for contracts & invoices"><?= e($s['company_address']) ?></textarea></div>
            <div class="form-row">
                <div class="form-group"><label>Phone</label><input name="company_phone" value="<?= e($s['company_phone']) ?>" placeholder="+91 98765 43210"></div>
            ...
            <div class="form-row">
                <div class="form-group"><label>Default Tax %</label><input type="number" step="0.01" name="default_tax_percent" value="<?= $s['default_tax_percent'] ?>"></div>
                <div class="form-group"><label>Currency</label>
                    <select name="currency"><option value="INR" <?= ($s['currency']??'INR')==='INR'?'selected':'' ?>>INR (₹)</option><option value="USD" <?= ($s['currency']??'')==='USD'?'selected':'' ?>>USD ($)</option></select>
                </div>
            </div>
